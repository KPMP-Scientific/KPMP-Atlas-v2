...
namespace ReadWrite
{
    ...
    class ReadWriteClass
    {
        ...
        public static void Delete_directory_recursive_if_it_exists(string input_directory)
        {
            string directory = Path.GetDirectoryName(input_directory);
            if (!String.IsNullOrEmpty(directory))
            {
        ...
        public static List<T> ReadRawData_and_FillList<T>(StreamReader stream, ReadWriteOptions_base options, string file_name) where T : class
        {
            ...
            if (options.File_has_headline)
            {
                ...
                if (String.IsNullOrEmpty(headline)) { throw new Exception(file_name + " is empty."); }
        ...
        public static void Write_one_type_array_as_single_column<T>(T[] Array, string directory, string fileName)
        {
