...
Sample_meta=readtable('aTAL_pseudobulk/Samples_Conditions_and_cell_numbers.csv');
index=find(Sample_meta.Cells>150);
...
Peak_sc_diff_all = readtable('aTAL_pseudobulk/All_samples_TAL_ATAC_markers.csv');
...
RNA_sc_diff_all = readtable('aTAL_pseudobulk/All_samples_TAL_RNA_markers.csv');
...
for p=1:length(Candidate_clusters) 
    ATAC = readtable(['aTAL_pseudobulk/Pseudo_bulk_TAL_ATAC_count_',Candidate_clusters{p},'_All_samples.txt'], 'ReadVariableNames',true);
    ...
    RNA = readtable(['aTAL_pseudobulk/Pseudo_bulk_TAL_RNA_count_',Candidate_clusters{p},'_All_samples.txt'], 'ReadVariableNames',true);
...
save('aTAL_trajectory_MAGICAL.mat')

load aTAL_trajectory_MAGICAL.mat

CollecTRI_regulons=readtable('CollecTRI_regulons.txt', 'delimiter', ',');

fid=fopen('V2_Kidney_aTAL_all_samples_trajectory_MAGICAL_circuits.txt', 'w');
...
similarity_matrix=zeros(10,10);
unique_matrix=zeros(10,1);
for p=1:10
...
%take out cluster D_16
index=find(MAGICAL_circuits.Gene_sc_Log2FC>0);
MAGICAL_circuits=MAGICAL_circuits(index,:);

index=find(strcmp(string(MAGICAL_circuits.Cluster_ID), 'D_16')==0);
MAGICAL_circuits=MAGICAL_circuits(index,:);


%lineage 4, up in 15
positive_index=find(strcmp(string(MAGICAL_circuits.Cluster_ID), 'D_15')>0);

negative_index=find(strcmp(string(MAGICAL_circuits.Cluster_ID), 'D_15')==0);
...
writetable(Lineage_positive_circuits, 'aTAL_lineage4.txt')


%Union lineage 2, up in 17, 6, 7, down in 15
positive_index=find(strcmp(string(MAGICAL_circuits.Cluster_ID), 'D_17')>0 |...
    strcmp(string(MAGICAL_circuits.Cluster_ID), 'D_6')>0 |...
    strcmp(string(MAGICAL_circuits.Cluster_ID), 'D_7')>0 );
...
writetable(Lineage_positive_circuits, 'aTAL_unionlineage.txt')



%take out union lineage and D_15
index=find(strcmp(string(MAGICAL_circuits.Cluster_ID), 'D_15')==0 &...
    strcmp(string(MAGICAL_circuits.Cluster_ID), 'D_17')==0 &...
    strcmp(string(MAGICAL_circuits.Cluster_ID), 'D_6')==0 &...
    strcmp(string(MAGICAL_circuits.Cluster_ID), 'D_7')==0 );
MAGICAL_circuits=MAGICAL_circuits(index,:);


%lineage 1, up in 11, 12, 23
positive_index=find(strcmp(string(MAGICAL_circuits.Cluster_ID), 'D_11')>0 |...
    strcmp(string(MAGICAL_circuits.Cluster_ID), 'D_12')>0 |...
    strcmp(string(MAGICAL_circuits.Cluster_ID), 'D_13')>0 );

negative_index=find(strcmp(string(MAGICAL_circuits.Cluster_ID), 'D_11')==0 &...
    strcmp(string(MAGICAL_circuits.Cluster_ID), 'D_12')==0 &...
    strcmp(string(MAGICAL_circuits.Cluster_ID), 'D_13')==0 );
...
writetable(Lineage_positive_circuits, 'aTAL_lineage1.txt')



%lineage 2, up in 5
positive_index=find(strcmp(string(MAGICAL_circuits.Cluster_ID), 'D_5')>0);

negative_index=find(strcmp(string(MAGICAL_circuits.Cluster_ID), 'D_5')==0);
...
writetable(Lineage_positive_circuits, 'aTAL_lineage2.txt')


%lineage 3, up in 8
positive_index=find(strcmp(string(MAGICAL_circuits.Cluster_ID), 'D_8')>0);

negative_index=find(strcmp(string(MAGICAL_circuits.Cluster_ID), 'D_8')==0);
...
writetable(Lineage_positive_circuits, 'aTAL_lineage3.txt')
