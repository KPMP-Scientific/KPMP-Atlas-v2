...
Sample_meta=readtable('Interstitial_Fibroblasts_pseudobulk/Samples_Conditions_and_cell_numbers_S9_S14.csv');
Sample_meta=Sample_meta(Sample_meta.Freq>90,:);
...
Peak_sc_diff_all = readtable('Interstitial_Fibroblasts_pseudobulk/All_samples_S9_S14_ATAC_markers.csv');
...
RNA_sc_diff_all = readtable('Interstitial_Fibroblasts_pseudobulk/All_samples_S9_S14_RNA_markers.csv');
...
for p=1:length(Candidate_clusters) 
    ATAC = readtable(['Interstitial_Fibroblasts_pseudobulk/Pseudo_bulk_9_14_count_',Candidate_clusters{p},'_All_samples.txt'], 'ReadVariableNames',true);
    ...
    RNA = readtable(['Interstitial_Fibroblasts_pseudobulk/Pseudo_bulk_9_14_RNA_count_',Candidate_clusters{p},'_All_samples.txt'], 'ReadVariableNames',true);
...
save('S9_S14_trajectory_MAGICAL.mat')

clear

load S9_S14_trajectory_MAGICAL.mat

CollecTRI_regulons=readtable('CollecTRI_regulons.txt', 'delimiter', ',');

fid=fopen('V2_Kidney_S9_S14_all_samples_trajectory_MAGICAL_circuits.txt', 'w');
...
MAGICAL_circuits=readtable('V2_Kidney_S9_S14_all_samples_trajectory_MAGICAL_circuits.txt');
...
selected_TFs=cell(1);
for p=1:length(Candidate_clusters)
    [xx,yy]=find(MAGICAL_post(p).TF_Peak_Binding_weight>0.8);
    if p==1
