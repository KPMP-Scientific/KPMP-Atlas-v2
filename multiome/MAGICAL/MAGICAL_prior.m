...
for i=1:length(xx)
    ...
    TF_ATAC_weight_1 = table2array(mdl.Coefficients(2,1)); 
    TF_ATAC_probability_1 = 1-table2array(mdl.Coefficients(2,4));

    [B,STATS] = robustfit(Candidate_TF_log2Count(yy(i),:)', Candidate_Peak_log2Count(xx(i),:)');
    TF_ATAC_weight_2 = B(2);
    TF_ATAC_probability_2 = 1-STATS.p(2);

    B_prior(xx(i),yy(i))=mean([TF_ATAC_weight_1, TF_ATAC_weight_2]);
    B_prob(xx(i),yy(i))=min(TF_ATAC_probability_1, TF_ATAC_probability_2);

%     [B_prior(xx(i), yy(i)),PVAL] = corr(Candidate_TF_log2Count(yy(i),:)', Candidate_Peak_log2Count(xx(i),:)', 'Type','Spearman');
%     B_prob(xx(i), yy(i))=1-PVAL;
end
B_mean=B_prior;
for t=1:T
...
for i=1:length(xx)
    ...
    L_prob(xx(i),yy(i))=min(ATAC_RNA_probability_1, ATAC_RNA_probability_2);
    
%     [L_prior(xx(i),yy(i)),PVAL] = corr(Candidate_Peak_log2Count(xx(i),:)', Candidate_Gene_log2Count(yy(i),:)', 'Type','Spearman');
%     L_prob(xx(i),yy(i))=1-PVAL;
