...
DimPlot(obj, reduction = "umap", label = TRUE, raster=FALSE,alpha = 0.4,
        ...
        ) + scale_color_manual(values = c("#0000FF4D","#9A4D424D","#009FFF4D")
        ) + NoLegend()
CellPropPlot(obj,
...
pdf(file='regulatory/scMEGA_S9-S12_ArchR-Trajectory.pdf',width=8,height=8)
print(p)
dev.off()
...
pdf(file='regulatory/scMEGA_SelectTFs_S9-S12_Heatmap.pdf',width=8,height=8)
draw(ht)
dev.off()

write.table(df.cor, file="regulatory/scMEGA_SelectTFs_S9-S12_Heatmap.txt", sep = "\t", row.names=TRUE, col.names=TRUE)
...
pdf(file='regulatory/scMEGA_SelectGenes_S9-S12_Heatmap.pdf',width=8,height=8)
draw(ht) 
dev.off()

write.table(df.p2g, file="regulatory/scMEGA_SelectGenes_S9-S12_Heatmap.txt", sep = "\t", row.names=TRUE, col.names=TRUE)
...
pdf(file='regulatory/scMEGA_SelectTFs_S9-S12_GRNHeatmap.pdf',width=18,height=6)
ht
dev.off()
...
write.table(df.grn, file="regulatory/scMEGA_TF2Peak2Gene_S9-S12_Table.txt", sep = "\t", row.names=TRUE, col.names=TRUE)
...
write.table(df.grn2, file="regulatory/scMEGA_TF2Peak2Gene_S9-S12_Table_cor0.4.txt", sep = "\t", row.names=TRUE, col.names=TRUE)
...
pdf(file='regulatory/scMEGA_SelectTFs_S9-S12_GRNPlot.pdf',width=10,height=10)
print(p)
dev.off()

save(obj, df.cor, df.grn, df.grn2, df.p2g, tf.gene.cor,
     file = "~/hsKidAt/blake_LTS/Atlas_V2/scratch/regulatory/scMEGA_S9-S12_Trajectory_GRN_0424-newData.RDA")
#load("~/hsKidAt/blake_LTS/Atlas_V2/scratch/regulatory/scMEGA_S9-S12_Trajectory_GRN_0424-newData.RDA")
...
pdf(file='regulatory/scMEGA_SelectTFs_S9-S12_Central-TFs.pdf',width=10,height=10)
#Network topological measures embedding: PC1 and PC2
p <- TopEmbGRN(df.grn=netobj)
#IRF1,RUNX1,STAT1,SOX4,THRB,ESR2,IRF2,BACH1,KLF10,NFKB2,NFKB1,RELB,FOXP1,GLI3,SMAD3,PRRX1 

#Network topological measures embedding: PC2 and PC3
p <- TopEmbGRN(df.grn=netobj,axis=c(2,3))
#THRB,ATF3,NR2F1,RUNX1,IRF1,STAT1,ESR2,BACH1,KLF10,GLI3,PRRX1,CREM

dev.off()

###
p1 <- PseudotimePlot(object = obj, tf.use = "IRF1")
p2 <- PseudotimePlot(object = obj, tf.use = "STAT1")

p1 + p2

p3 <- PseudotimePlot(object = obj, tf.use = "THRB")
p4 <- PseudotimePlot(object = obj, tf.use = "KLF10")

p3 + p4

p5 <- PseudotimePlot(object = obj, tf.use = "RUNX1")
p6 <- PseudotimePlot(object = obj, tf.use = "NFKB2")

p5 + p6

p7 <- PseudotimePlot(object = obj, tf.use = "RFX2")
p8 <- PseudotimePlot(object = obj, tf.use = "ATF3")

p7 + p8

pdf(file='regulatory/scMEGA_SelectTFs_S9-S12_select-TFs_Pseudotime.pdf',width=20,height=7.5)
...
write.table(df.cor, file="regulatory/scMEGA_SelectTFs_S9-S12_Heatmap_Network_Scores_Cor0-4.txt", sep = "\t", row.names=TRUE, col.names=TRUE)
