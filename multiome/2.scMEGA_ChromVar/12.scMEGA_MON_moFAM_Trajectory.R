...
DimPlot(obj, reduction = "umap", label = TRUE, raster=FALSE,alpha = 0.4,
        ...
        ) + scale_color_manual(values = c("#00FF00","#FE8F42","#FFD300")
        ) + NoLegend()
CellPropPlot(obj,
...
obj <- AddTrajectory(object = obj, 
                     trajectory = c("MON","moMAC-HBEGF+","moFAM"),
                     group.by = "v2.subclass.l3", 
                     reduction = "umap",
                     use.all = FALSE)
...
pdf(file='regulatory/scMEGA_MON-moFAM_ArchR-Trajectory.pdf',width=6,height=6)
print(p)
dev.off()
...
pdf(file='regulatory/scMEGA_SelectTFs_MON-moFAM_Heatmap.pdf',width=8,height=8)
draw(ht)
dev.off()

write.table(df.cor, file="regulatory/scMEGA_SelectTFs_MON-moFAM_Heatmap.txt", sep = "\t", row.names=TRUE, col.names=TRUE)
...
pdf(file='regulatory/scMEGA_SelectGenes_MON-moFAM_Heatmap.pdf',width=8,height=8)
draw(ht) 
dev.off()

write.table(df.p2g, file="regulatory/scMEGA_SelectGenes_MON-moFAM_Heatmap.txt", sep = "\t", row.names=TRUE, col.names=TRUE)
...
pdf(file='regulatory/scMEGA_SelectTFs_MON-moFAM_GRNHeatmap.pdf',width=18,height=6)
ht
dev.off()
...
write.table(df.grn, file="regulatory/scMEGA_TF2Peak2Gene_MON-moFAM_Table.txt", sep = "\t", row.names=TRUE, col.names=TRUE)
...
write.table(df.grn2, file="regulatory/scMEGA_TF2Peak2Gene_MON-moFAM_Table_cor0.4.txt", sep = "\t", row.names=TRUE, col.names=TRUE)
...
pdf(file='regulatory/scMEGA_SelectTFs_MON-moFAM_GRNPlot.pdf',width=10,height=10)
print(p)
dev.off()

save(obj, df.cor, df.grn, df.grn2, df.p2g, tf.gene.cor,
     file = "~/hsKidAt/blake_LTS/Atlas_V2/scratch/regulatory/scMEGA_MON-moFAM_Trajectory_GRN_0424-newData.RDA")
#load("~/hsKidAt/blake_LTS/Atlas_V2/scratch/regulatory/scMEGA_MON-moFAM_Trajectory_GRN_0424-newData.RDA")
...
pdf(file='regulatory/scMEGA_SelectTFs_MON-moFAM_Central-TFs.pdf',width=10,height=10)
#Network topological measures embedding: PC1 and PC2
p <- TopEmbGRN(df.grn=netobj)
#USF2,EGR2,MITF,STAT1,TFEB,TFEC,IRF8

#Network topological measures embedding: PC2 and PC3
p <- TopEmbGRN(df.grn=netobj,axis=c(2,3))
#USF2,EGR2,TFEC,TFEB,MITF,STAT1,NFE2L1,ATF3,IRF8,KLF6,MAF

dev.off()

###
p1 <- PseudotimePlot(object = obj, tf.use = "USF2")
p2 <- PseudotimePlot(object = obj, tf.use = "EGR2")

p1 + p2

p3 <- PseudotimePlot(object = obj, tf.use = "TFEC")
p4 <- PseudotimePlot(object = obj, tf.use = "MITF")

p3 + p4

p5 <- PseudotimePlot(object = obj, tf.use = "NFE2L1")
p6 <- PseudotimePlot(object = obj, tf.use = "STAT1")

p5 + p6

p7 <- PseudotimePlot(object = obj, tf.use = "IRF8")
p8 <- PseudotimePlot(object = obj, tf.use = "MAF")

p7 + p8

pdf(file='regulatory/scMEGA_SelectTFs_MON-moFAM_select-TFs_Pseudotime.pdf',width=20,height=7.5)
...
write.table(df.cor, file="regulatory/scMEGA_SelectTFs_MON-moFAM_Heatmap_Network_Scores_Cor0-4.txt", sep = "\t", row.names=TRUE, col.names=TRUE)
