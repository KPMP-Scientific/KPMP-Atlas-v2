...
load("~/hsKidAt/blake_LTS/Atlas_V2/Intermediate_Objects/Kidney_AtlasV2_Seurat_distal-tubules_subset_filtered_aTAL_Lineage3_0424-newData_TALA.rda")
TAL.R <- KB.TAL
...
DimPlot(obj, reduction = "umap", label = TRUE, raster=FALSE,alpha = 0.4,
        ...
        ) + scale_color_manual(values = c("#FE8F42","#DC5E93","#FFD300")
        ) + NoLegend()
CellPropPlot(obj,
...
TrajectoryPlot(object = obj, 
               reduction = "umap",
               trajectory = "pseudotime.lineage3",
...
obj <- AddTrajectory(object = obj, 
                     trajectory = c("aTAL1","aTAL2","frTAL"),
                     group.by = "v2.subclass.l3", 
                     reduction = "umap",
                     use.all = FALSE)
...
pdf(file='regulatory/scMEGA_aTAL-frTAL_ArchR-Trajectory_TALA.pdf',width=8,height=8)
print(p)
dev.off()
...
pdf(file='regulatory/scMEGA_SelectTFs_aTAL-frTAL_Heatmap_TALA.pdf',width=8,height=8)
draw(ht)
dev.off()

write.table(df.cor, file="regulatory/scMEGA_SelectTFs_aTAL-frTAL_Heatmap_TALA.txt", sep = "\t", row.names=TRUE, col.names=TRUE)
...
pdf(file='regulatory/scMEGA_SelectGenes_aTAL-frTAL_Heatmap_TALA.pdf',width=8,height=8)
draw(ht) 
dev.off()

write.table(df.p2g, file="regulatory/scMEGA_SelectGenes_aTAL-frTAL_Heatmap_TALA.txt", sep = "\t", row.names=TRUE, col.names=TRUE)
...
pdf(file='regulatory/scMEGA_SelectTFs_aTAL-frTAL_GRNHeatmap_TALA.pdf',width=18,height=6)
ht
dev.off()
...
write.table(df.grn, file="regulatory/scMEGA_TF2Peak2Gene_aTAL-frTAL_Table_TALA.txt", sep = "\t", row.names=TRUE, col.names=TRUE)
...
write.table(df.grn2, file="regulatory/scMEGA_TF2Peak2Gene_aTAL-frTAL_Table_cor0.4_TALA.txt", sep = "\t", row.names=TRUE, col.names=TRUE)
...
pdf(file='regulatory/scMEGA_SelectTFs_aTAL-frTAL_GRNPlot_TALA.pdf',width=10,height=10)
print(p)
dev.off()

save(obj, df.cor, df.grn, df.grn2, df.p2g, tf.gene.cor,
     file = "~/hsKidAt/blake_LTS/Atlas_V2/scratch/regulatory/scMEGA_aTAL-frTAL_Trajectory_GRN_0424-newData_TALA.RDA")
#load("~/hsKidAt/blake_LTS/Atlas_V2/scratch/regulatory/scMEGA_aTAL-frTAL_Trajectory_GRN_0424-newData_TALA.RDA")
...
pdf(file='regulatory/scMEGA_SelectTFs_aTAL-frTAL_Central-TFs_TALA.pdf',width=10,height=10)
#Network topological measures embedding: PC1 and PC2
p <- TopEmbGRN(df.grn=netobj)
#FOXC1, ELF3, KLF2, KLF13, NFIA, EHF, EGR1, HNF4G, HOXB6, HOXC9, 
#JUN,KLF5, SOX4, POU3F3,  TCF7L1

#Network topological measures embedding: PC2 and PC3
p <- TopEmbGRN(df.grn=netobj,axis=c(2,3))
#JUN, TFAP2B, SOX4, KLF2, HLF, ELF3, EGR1, NFYC,  HOXB3

dev.off()

###
p1 <- PseudotimePlot(object = obj, tf.use = "KLF2")
p2 <- PseudotimePlot(object = obj, tf.use = "JUN")

p1 + p2

p3 <- PseudotimePlot(object = obj, tf.use = "ELF3")
p4 <- PseudotimePlot(object = obj, tf.use = "SOX9")

p3 + p4

p5 <- PseudotimePlot(object = obj, tf.use = "FOXC1")
p6 <- PseudotimePlot(object = obj, tf.use = "TFAP2B")

p5 + p6

p7 <- PseudotimePlot(object = obj, tf.use = "SOX4")
p8 <- PseudotimePlot(object = obj, tf.use = "NFIA")

p7 + p8

p9 <- PseudotimePlot(object = obj, tf.use = "ESRRB")

pdf(file='regulatory/scMEGA_SelectTFs_aTAL-frTAL_select-TFs_Pseudotime_TALA.pdf',width=20,height=7.5)
...
write.table(df.cor, file="regulatory/scMEGA_SelectTFs_aTAL-frTAL_Heatmap_Network_Scores_Cor0-4_TALA.txt", sep = "\t", row.names=TRUE, col.names=TRUE)
