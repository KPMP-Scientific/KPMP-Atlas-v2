...
TrajectoryPlot(object = obj, 
               reduction = "umap",
               trajectory = "pseudotime.lineage2",
...
obj <- AddTrajectory(object = obj, 
                     trajectory = c("aPT2","aPT1","aPT-S1/S2","frPT-S1/S2"),
                     group.by = "v2.subclass.l3", 
                     reduction = "umap",
                     use.all = FALSE)
...
pdf(file='regulatory/scMEGA_aPT-S1-2_ArchR-Trajectory.pdf',width=8,height=8)
print(p)
dev.off()
...
pdf(file='regulatory/scMEGA_SelectTFs_aPT-S1-2_Heatmap.pdf',width=8,height=8)
draw(ht)
dev.off()

write.table(df.cor, file="regulatory/scMEGA_SelectTFs_aPT-S1-2_Heatmap.txt", sep = "\t", row.names=TRUE, col.names=TRUE)
...
pdf(file='regulatory/scMEGA_SelectGenes_aPT-S1-2_Heatmap.pdf',width=8,height=8)
draw(ht) 
dev.off()

write.table(df.p2g, file="regulatory/scMEGA_SelectGenes_aPT-S1-2_Heatmap.txt", sep = "\t", row.names=TRUE, col.names=TRUE)
...
pdf(file='regulatory/scMEGA_SelectTFs_aPT-S1-2_GRNHeatmap.pdf',width=18,height=6)
ht
dev.off()
...
write.table(df.grn, file="regulatory/scMEGA_TF2Peak2Gene_aPT-S1-2_Table.txt", sep = "\t", row.names=TRUE, col.names=TRUE)
...
write.table(df.grn2, file="regulatory/scMEGA_TF2Peak2Gene_aPT-S1-2_Table_cor0.4.txt", sep = "\t", row.names=TRUE, col.names=TRUE)
...
pdf(file='regulatory/scMEGA_SelectTFs_aPT-S1-2_GRNPlot.pdf',width=10,height=10)
print(p)
dev.off()

save(obj, df.cor, df.grn, df.grn2, df.p2g, tf.gene.cor,
     file = "~/hsKidAt/blake_LTS/Atlas_V2/scratch/regulatory/scMEGA_aPT-S1-2_Trajectory_GRN_0424-newData.RDA")
...
pdf(file='regulatory/scMEGA_SelectTFs_aPT-S1-2_Central-TFs.pdf',width=10,height=10)
#Network topological measures embedding: PC1 and PC2
p <- TopEmbGRN(df.grn=netobj)
#KLF6, JUND, GRHL2,ETS1,SOX4,JUN,FOSL2,SOX9,VEZF1,JUNB,EGR1,FOXC1,ELF3,THAP11,YY1,
#MEF2C,ONECUT1,MEIS3,MNX1,STAT1,HES1,NFKB1

#Network topological measures embedding: PC2 and PC3
p <- TopEmbGRN(df.grn=netobj,axis=c(2,3))
#GRHL2,HOXB7,KLF6,SOX4,FOSL2,ETS1,JUN,JUNB,RUNX2,EGR1,YY1,THAP11
...
p3 <- PseudotimePlot(object = obj, tf.use = "MEF2C")
...
pdf(file='regulatory/scMEGA_SelectTFs_aPT-S1-2_select-TFs_Pseudotime.pdf',width=20,height=7.5)
...
write.table(df.cor, file="regulatory/scMEGA_SelectTFs_aPT-S1-2_Heatmap_Network_Scores_Cor0-4.txt", sep = "\t", row.names=TRUE, col.names=TRUE)
...
pdf(file='regulatory/scMEGA_aPT-S1-2_ArchR-Trajectory_clusters_labeled.pdf',width=8,height=6)
...
dev.off()
pdf(file='regulatory/scMEGA_aPT-S1-2_ArchR-Trajectory_clusters_unlabeled.pdf',width=8,height=6)
