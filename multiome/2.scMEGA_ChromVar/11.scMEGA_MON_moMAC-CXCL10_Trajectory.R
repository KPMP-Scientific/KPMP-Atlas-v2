...
DimPlot(obj, reduction = "umap", label = TRUE, raster=FALSE,alpha = 0.4,
        ...
        ) + scale_color_manual(values = c("#FE8F42","#C8FF00")
        ) + NoLegend()
CellPropPlot(obj,
...
pdf(file='regulatory/scMEGA_MON-CXCL10_ArchR-Trajectory.pdf',width=6,height=6)
print(p)
dev.off()
...
pdf(file='regulatory/scMEGA_SelectTFs_MON-CXCL10_Heatmap.pdf',width=8,height=8)
draw(ht)
dev.off()

write.table(df.cor, file="regulatory/scMEGA_SelectTFs_MON-CXCL10_Heatmap.txt", sep = "\t", row.names=TRUE, col.names=TRUE)
...
pdf(file='regulatory/scMEGA_SelectGenes_MON-CXCL10_Heatmap.pdf',width=8,height=8)
draw(ht) 
dev.off()

write.table(df.p2g, file="regulatory/scMEGA_SelectGenes_MON-CXCL10_Heatmap.txt", sep = "\t", row.names=TRUE, col.names=TRUE)
...
pdf(file='regulatory/scMEGA_SelectTFs_MON-CXCL10_GRNHeatmap.pdf',width=18,height=6)
ht
dev.off()
...
write.table(df.grn, file="regulatory/scMEGA_TF2Peak2Gene_MON-CXCL10_Table.txt", sep = "\t", row.names=TRUE, col.names=TRUE)
...
write.table(df.grn2, file="regulatory/scMEGA_TF2Peak2Gene_MON-CXCL10_Table_cor0.4.txt", sep = "\t", row.names=TRUE, col.names=TRUE)
...
pdf(file='regulatory/scMEGA_SelectTFs_MON-CXCL10_GRNPlot.pdf',width=10,height=10)
print(p)
dev.off()

save(obj, df.cor, df.grn, df.grn2, df.p2g, tf.gene.cor,
     file = "~/hsKidAt/blake_LTS/Atlas_V2/scratch/regulatory/scMEGA_MON-CXCL10_Trajectory_GRN_0424-newData.RDA")
#load("~/hsKidAt/blake_LTS/Atlas_V2/scratch/regulatory/scMEGA_MON-CXCL10_Trajectory_GRN_0424-newData.RDA")
...
pdf(file='regulatory/scMEGA_SelectTFs_MON-CXCL10_Central-TFs.pdf',width=10,height=10)
#Network topological measures embedding: PC1 and PC2
p <- TopEmbGRN(df.grn=netobj)
#IRF1,IRF8,NFKB1,STAT1,RELB,CCL3,REL

#Network topological measures embedding: PC2 and PC3
p <- TopEmbGRN(df.grn=netobj,axis=c(2,3))
#IRF1,NFKB1,STAT1,IRF8,NFATC2,NR2F2,REL,IRF2,IRF9,IRF7,EGR3,RELB

dev.off()

###
p1 <- PseudotimePlot(object = obj, tf.use = "NFE2L1")
p2 <- PseudotimePlot(object = obj, tf.use = "NR2F2")

p1 + p2

p3 <- PseudotimePlot(object = obj, tf.use = "IRF1")
p4 <- PseudotimePlot(object = obj, tf.use = "NFKB1")

p3 + p4

p5 <- PseudotimePlot(object = obj, tf.use = "STAT1")
p6 <- PseudotimePlot(object = obj, tf.use = "IRF8")

p5 + p6

p7 <- PseudotimePlot(object = obj, tf.use = "REL")
p8 <- PseudotimePlot(object = obj, tf.use = "NFATC2")

p7 + p8

pdf(file='regulatory/scMEGA_SelectTFs_MON-CXCL10_select-TFs_Pseudotime.pdf',width=20,height=7.5)
...
write.table(df.cor, file="regulatory/scMEGA_SelectTFs_MON-CXCL10_Heatmap_Network_Scores_Cor0-4.txt", sep = "\t", row.names=TRUE, col.names=TRUE)
