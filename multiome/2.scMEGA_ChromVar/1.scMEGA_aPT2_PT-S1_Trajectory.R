...
DimPlot(obj, reduction = "umap", label = TRUE, raster=FALSE,alpha = 0.4,
        ...
        ) + scale_color_manual(values = c("#00FFBE","#1F9698","#B1CC71","#00479E")
        ) + NoLegend()
CellPropPlot(obj,
...
DimPlot(obj, reduction = "dm", label = TRUE, raster=FALSE,alpha = 0.4,
        label.size = 6, repel = TRUE, pt.size = 1) + ggtitle("Subclass l3"
        ) + scale_color_manual(values = c("#1F9698","#FE8F42","#DC5E93","#B1CC71")
        ) + NoLegend()
...
obj <- AddTrajectory(object = obj, 
                     trajectory = c("aPT2","aPT1","aPT-S1/S2","PT-S1"),
                     group.by = "v2.subclass.l3", 
                     reduction = "umap",
                     use.all = FALSE)
...
pdf(file='regulatory/scMEGA_aPT-S1-2_l1_ArchR-Trajectory.pdf',width=8,height=8)
print(p)
dev.off()
...
pdf(file='regulatory/scMEGA_SelectTFs_aPT-S1-2_l1_Heatmap.pdf',width=8,height=8)
draw(ht)
dev.off()

write.table(df.cor, file="regulatory/scMEGA_SelectTFs_aPT-S1-2_l1_Heatmap.txt", sep = "\t", row.names=TRUE, col.names=TRUE)
...
pdf(file='regulatory/scMEGA_SelectGenes_aPT-S1-2_l1_Heatmap.pdf',width=8,height=8)
draw(ht) 
dev.off()

write.table(df.p2g, file="regulatory/scMEGA_SelectGenes_aPT-S1-2_l1_Heatmap.txt", sep = "\t", row.names=TRUE, col.names=TRUE)
...
pdf(file='regulatory/scMEGA_SelectTFs_aPT-S1-2_l1_GRNHeatmap.pdf',width=18,height=6)
ht
dev.off()
...
write.table(df.grn, file="regulatory/scMEGA_TF2Peak2Gene_aPT-S1-2_l1_Table.txt", sep = "\t", row.names=TRUE, col.names=TRUE)
...
write.table(df.grn2, file="regulatory/scMEGA_TF2Peak2Gene_aPT-S1-2_l1_Table_cor0.4.txt", sep = "\t", row.names=TRUE, col.names=TRUE)
...
pdf(file='regulatory/scMEGA_SelectTFs_aPT-S1-2_l1_GRNPlot.pdf',width=10,height=10)
print(p)
dev.off()

save(obj, df.cor, df.grn, df.grn2, df.p2g, tf.gene.cor,
     file = "~/hsKidAt/blake_LTS/Atlas_V2/scratch/regulatory/scMEGA_aPT-S1-2_l1_Trajectory_GRN_0424-newData.RDA")
...
pdf(file='regulatory/scMEGA_SelectTFs_aPT-S1-2_l1_Central-TFs.pdf',width=10,height=10)
#Network topological measures embedding: PC1 and PC2
p <- TopEmbGRN(df.grn=netobj)
#FOXC1, SOX4,ELF3,STAT1,FOSL2,SOX9,IRF1,RUNX2,POU3F3,MYC,HES1,KLF6,NFKB1,HIF1A,ARNT2,VEZF1,ATF3
#HOXB7,NFKB2,RELB,BACH2

#Network topological measures embedding: PC2 and PC3
p <- TopEmbGRN(df.grn=netobj,axis=c(2,3))
#MSANTD3,KLF6,ELF3,IRF1,NFKB1,HOXB7,JUN,STAT1,POU3F3,VEZF1,NFKB2,EGR1,ETS1,HES1,NFAT5,
#ESR2,ATF3,RELB,HIF1A,HNF4A,BHLHE40

dev.off()

###
p1 <- PseudotimePlot(object = obj, tf.use = "KLF6")
p2 <- PseudotimePlot(object = obj, tf.use = "SOX9")

p1 + p2

p3 <- PseudotimePlot(object = obj, tf.use = "MSANTD3")
p4 <- PseudotimePlot(object = obj, tf.use = "MEIS3")
...
p8 <- PseudotimePlot(object = obj, tf.use = "HES1")

p7 + p8

pdf(file='regulatory/scMEGA_SelectTFs_aPT-S1-2_l1_select-TFs_Pseudotime.pdf',width=20,height=7.5)
...
write.table(df.cor, file="regulatory/scMEGA_SelectTFs_aPT-S1-2_l1_Heatmap_Network_Scores_Cor0-4.txt", sep = "\t", row.names=TRUE, col.names=TRUE)
...
pdf(file='regulatory/scMEGA_aPT-S1-2_l1_ArchR-Trajectory_clusters_labeled.pdf',width=8,height=6)
...
dev.off()
pdf(file='regulatory/scMEGA_aPT-S1-2_l1_ArchR-Trajectory_clusters_unlabeled.pdf',width=8,height=6)
