...
DimPlot(obj, reduction = "umap", label = TRUE, raster=FALSE,alpha = 0.4,
        ...
        ) + scale_color_manual(values = c("#14F9FF","#1F9698","#0000FF","#B1CC71")
        ) #+ NoLegend()
CellPropPlot(obj,
...
obj <- AddTrajectory(object = obj, 
                     trajectory = c("aPT2","aPT1","aPT-S3","PT-S3"),
                     group.by = "v2.subclass.l3", 
                     reduction = "umap",
                     use.all = FALSE)
...
pdf(file='regulatory/scMEGA_aPT-PTS3_l4_ArchR-Trajectory.pdf',width=8,height=8)
print(p)
dev.off()
...
pdf(file='regulatory/scMEGA_SelectTFs_aPT-PTS3_l4_Heatmap.pdf',width=8,height=8)
draw(ht)
dev.off()

write.table(df.cor, file="regulatory/scMEGA_SelectTFs_aPT-PTS3_l4_Heatmap.txt", sep = "\t", row.names=TRUE, col.names=TRUE)
...
pdf(file='regulatory/scMEGA_SelectGenes_aPT-PTS3_l4_Heatmap.pdf',width=8,height=8)
draw(ht) 
dev.off()

write.table(df.p2g, file="regulatory/scMEGA_SelectGenes_aPT-PTS3_l4_Heatmap.txt", sep = "\t", row.names=TRUE, col.names=TRUE)
...
pdf(file='regulatory/scMEGA_SelectTFs_aPT-PTS3_l4_GRNHeatmap.pdf',width=18,height=6)
ht
dev.off()
...
write.table(df.grn, file="regulatory/scMEGA_TF2Peak2Gene_aPT-PTS3_l4_Table.txt", sep = "\t", row.names=TRUE, col.names=TRUE)
...
write.table(df.grn2, file="regulatory/scMEGA_TF2Peak2Gene_aPT-PTS3_l4_Table_cor0.4.txt", sep = "\t", row.names=TRUE, col.names=TRUE)
...
pdf(file='regulatory/scMEGA_SelectTFs_aPT-PTS3_l4_GRNPlot.pdf',width=10,height=10)
print(p)
dev.off()

save(obj, df.cor, df.grn, df.grn2, df.p2g, tf.gene.cor,
     file = "~/hsKidAt/blake_LTS/Atlas_V2/scratch/regulatory/scMEGA_aPT-PTS3_l4_Trajectory_GRN_0424-newData.RDA")
#load("~/hsKidAt/blake_LTS/Atlas_V2/scratch/regulatory/scMEGA_aPT-PTS3_l4_Trajectory_GRN_0424-newData.RDA")
...
pdf(file='regulatory/scMEGA_SelectTFs_aPT-PTS3_l4_Central-TFs.pdf',width=10,height=10)
#Network topological measures embedding: PC1 and PC2
p <- TopEmbGRN(df.grn=netobj)
#RUNX1,KLF6,VEZF1,MYC,IRF1,STAT1,JUN,SOX4,ATF4,HES1,ETS1,

#Network topological measures embedding: PC2 and PC3
p <- TopEmbGRN(df.grn=netobj,axis=c(2,3))
#NFIX,MYC,RUNX1,RUNX2,ZEB1

dev.off()

###
p1 <- PseudotimePlot(object = obj, tf.use = "STAT1")
p2 <- PseudotimePlot(object = obj, tf.use = "SOX9")

p1 + p2

p3 <- PseudotimePlot(object = obj, tf.use = "ELF3")
p4 <- PseudotimePlot(object = obj, tf.use = "MEIS1")

p3 + p4

p5 <- PseudotimePlot(object = obj, tf.use = "HOXB7")
p6 <- PseudotimePlot(object = obj, tf.use = "IRF1")

p5 + p6

p7 <- PseudotimePlot(object = obj, tf.use = "HNF4A")
p8 <- PseudotimePlot(object = obj, tf.use = "NFKB2")

p7 + p8

pdf(file='regulatory/scMEGA_SelectTFs_aPT-PTS3_l4_select-TFs_Pseudotime.pdf',width=20,height=7.5)
...
write.table(df.cor, file="regulatory/scMEGA_SelectTFs_aPT-PTS3_l4_Heatmap_Network_Scores_Cor0-4.txt", sep = "\t", row.names=TRUE, col.names=TRUE)
...
pdf(file='regulatory/scMEGA_aPT-PTS3_l4_ArchR-Trajectory_clusters_labeled.pdf',width=8,height=6)
...
dev.off()
pdf(file='regulatory/scMEGA_aPT-PTS3_l4_ArchR-Trajectory_clusters_unlabeled.pdf',width=8,height=6)
